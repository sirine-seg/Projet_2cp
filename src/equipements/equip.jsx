...
const EquipementsPage = () => {
  ...
  async function fetchEquipements(search = searchTerm, currentFilters = filters) {
    ...
    if (cachedEquipements[cacheKey]) {
      ...
      const filteredData = cachedEquipements[cacheKey].filter(equip => filterEquipement(equip, currentFilters, search));
      setFilteredEquipements(filteredData);
      setDisplayedEquipements(filteredData.slice(0, visibleCount));
      return;
    }
    ...
    try {
      ...
      const filteredData = data.filter(equip => filterEquipement(equip, currentFilters, search));
      setFilteredEquipements(filteredData);
      setDisplayedEquipements(filteredData.slice(0, visibleCount));
    } catch (error) {
    ...
    const fetchOptions = async () => {
      try {
        const [typeRes, catRes, locRes] = await Promise.all([
          fetch("http://127.0.0.1:8000/api/equipements/type/", { headers: { Authorization: `Bearer ${token}` } }),
          fetch("http://127.0.0.1:8000/api/equipements/categorie/", { headers: { Authorization: `Bearer ${token}` } }),
          fetch("http://127.0.0.1:8000/api/equipements/localisation/", { headers: { Authorization: `Bearer ${token}` } }),
        ]);
  ...
  const previousEtat = async (id_equipement, setEtat) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Erreur lors de la récupération de l'état :", data.detail || data);
      }
    } catch (error) {
  ...
  <div>
    <AjouterButton onClick={() => navigate("/AjouterEquipement")} />
  </div>
)}
            </div>
          </div>
                  ...
                  <div key={equipement.id_equipement} className="relative">
                    ...
                    {!loading && menuOpen === equipement.id_equipement && (
                      ...
                      >
                        ...
                        <button
                          className="block w-full text-left px-4 py-2 hover:bg-gray-200"
                          onClick={() => navigate(`/DetailsEquipement/${equipement.id_equipement}`)}
                        >
                          Détails
                        </button>
                      </div>
