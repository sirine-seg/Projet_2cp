      ...
      //  };
        ...
        const handleClick = (user) => {
            if (user.role === "Technicien") {
                navigate(`/TechnicienDetailstt/${user.id}`); // Redirige vers la page spéciale pour les techniciens
            } else {
                navigate(`/utilisateurDetailstt/${user.id}`); // Redirige vers AjoutPage
            }
        };
...
<div className="absolute top-6 right-16 flex items-center space-x-3">
        ...
        <span className="text-white text-lg font-semibold">User Name</span>
                ...
                <div className="w-full max-w-7xl bg-darck min-h-screen rounded-t-[80px] px-6 py-8 shadow-md flex flex-col">
                  {/* bare de recherhce  */}
