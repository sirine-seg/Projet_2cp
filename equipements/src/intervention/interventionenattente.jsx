...
useEffect(() => {
    const fetchintervention = async () => {
        try {
            ...
            const filtered = data.filter((item) => item.statut_label?.trim() == "En attente");
    ...
    useEffect(() => {
        ...
        const handleDisponibleClick = () => {
            ...
            navigate("/interventions-en-attente");
        };
...
<div className="absolute top-6 right-16 flex items-center space-x-3">
    ...
    <div className="flex items-center space-x-4"> 
        {/* Bouton Disponible */}
        <button
        className="px-4 py-2 rounded-lg shadow-md bg-ddd" 
      ...
      en attente
        ...
        {/* Bouton Ajouter */}
                    ...
                    {visibleCount < interventions.length && (
                       ...
                       <div className="flex flex-wrap justify-start gap-x-6 gap-y-7 w-full max-w-6xl px-4 mt-6 ml-4">
...
<span className={`px-2 py-1 text-xs font-bold text-white rounded-full ${
  statusColors[intervention.statut_label] 
}`}>
            ...
            {menuOpen === intervention.id && (
                                <div className="absolute top-10 right-2 bg-white shadow-lg rounded-md text-black w-64 z-50">
                                    <button className="block w-full text-left px-4 py-2 hover:bg-gray-200"
                                      onClick={(event) => {
                                        event.stopPropagation(); // ← empêche le clic d'atteindre la carte
                                    ...
                                    onClick={(event) => {
                                        event.stopPropagation(); 
                                        navigate(`/Affecterintervention/${intervention.id}`);
                                    ...
                                    <button
                                        ...
                                        onClick={(event) => {
                                            event.stopPropagation(); // ← encore ici
  ...
  className="block w-full text-left px-4 py-2 hover:bg-gray-200"
  onClick={(event) => {
    event.stopPropagation(); // ← ici aussi pour éviter le bug
    setSelectedInterventionid(intervention.id);
...
{isDeletePopupVisible && (
  <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-40 z-50">
    <div className="bg-white p-8 rounded-3xl shadow-2xl w-[90%] max-w-md relative text-center">
      ...
      {/* Icône cercle gris clair avec point d'exclamation rouge */}
      <div className="mx-auto mb-4 w-16 h-16 rounded-full bg-gray-200 flex items-center justify-center">
      ...
      {/* Texte principal */}
      <p className="text-xl font-semibold text-gray-900">Êtes-vous sûr de vouloir supprimer cette intervention ?</p>
      ...
      <div className="mt-6 flex justify-center">
        <button
          ...
          className="px-6 py-2 rounded-full bg-[#F09C0A] text-white "
        >
          Supprimer
        </button>
      </div>
...
{isSuccessPopupVisible && (
  <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-40 z-50">
    <div className="bg-white p-8 rounded-3xl shadow-2xl w-[90%] max-w-md relative text-center">
      ...
      {/* Texte principal */}
      <p className="text-xl font-semibold text-gray-900">L'intervention a ete modifier avec succes   !</p>
