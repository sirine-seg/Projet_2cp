  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => { // Simuler le temps de chargement
        setLoading(false);
        if (response.ok) {
          localStorage.setItem("accessToken", data.access);  //   stocker un jeton d'accès (accessToken) dans le stockage local du navigateur après une authentification réussie.
